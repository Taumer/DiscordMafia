FROM mcr.microsoft.com/dotnet/sdk:2.1-alpine AS build-env
...
FROM mcr.microsoft.com/dotnet/runtime:2.1-alpine as bot
COPY --from=build-env /opt/sources/DiscordMafia/out /app
WORKDIR /app
ENTRYPOINT ["dotnet", "DiscordMafia.dll"]

FROM mcr.microsoft.com/dotnet/runtime:2.1-alpine as web
